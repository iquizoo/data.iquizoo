...
- Fix parsing of `prep_fun_name`, and safer handling of `input` /
  `extra` and parallel forms.
...
- Removed `rtsd` index from `TOVA` related games. This index is highly
  correlated with `dprime` index.
...
- Added `...` to pass additional arguments to
  [`merge()`](https://rdrr.io/r/base/merge.html) in
  [`merge_preproc()`](https://iquizoo.github.io/data.iquizoo/reference/merge_preproc.md).

## data.iquizoo 2024.03.31-1

- Removed `screen_indices()` function.
- Renamed `match_preproc()` as
  [`merge_preproc()`](https://iquizoo.github.io/data.iquizoo/reference/merge_preproc.md).
- Now dplyr is not required in the package.

## data.iquizoo 2024.03.31

- Let missing values in `game_name_ver` column of `game_info` be empty
  strings (`""`).

## data.iquizoo 2024.03.29-1

- Add `fail_on_missing` to
  [`match_info()`](https://iquizoo.github.io/data.iquizoo/reference/match_info.md)
  to control whether to fail when found unmatched ids.
...
- Added
  [`match_info()`](https://iquizoo.github.io/data.iquizoo/reference/match_info.md)
  to extract different pieces of game information.

## data.iquizoo 2023.12.08

- Support tagging specific pre-processing functions. When a
  pre-processing function is tagged, it signals that it is experimental
  and may not work as expected. This is useful for newly developed
  pre-processing functions.
- Add `rm_tagged` argument in `match_preproc()` to remove tagged
  pre-processing functions.

## data.iquizoo 2023.11.04

- Support `type` argument in `match_preproc()` to specify which type of
  join to use.
...
- Moved `prep_fun` column just after the `game_id` in the results
  calling `match_preproc()`.

## data.iquizoo 2023.10.22

### Breaking Changes

- Removed `prep_fun_name`, `input` and `extra` columns from `game_info`
  table.
- Added `match_preproc()` function to match pre-processing parameters
  for games.
...
- Added a new column to indicate whether the main index is reversed or
  not.

## data.iquizoo 2023.10.04

- Added a new column to set the name of the one main index for each
  task.

## data.iquizoo 2023.10.03

- Fix `prep_fun_name` for QA version of Face Name task. Now it uses
  `preproc.iquizoo::countcorrect()`.
...
- Changed version to `YYYY.MM.DD` format. This will make the version
  number more informative about the release date.
...
- Support `VersionName` update with a new column named `"version_name"`
  in `game_info` table.
...
- Changed `game_id` class to
  [`bit64::integer64`](https://rdrr.io/pkg/bit64/man/bit64-package.html).
  This is the default class of the values fetched from database for
  these values.
...
- Store `input` and `extra` as strings by
  [`rlang::expr_text()`](https://rlang.r-lib.org/reference/expr_label.html).

## data.iquizoo 0.1.0

### Breaking Changes

- Remove obsolete data `config_prep_fun` and merged its info to
  `game_info`. Basically, `input` and `extra` can be configured there.
...
- Added `config_prep_fun` as a default and example configuration of
  pre-processing functions.
...
- Added `game_info` dataset to store correponding pre-processing
  functions for games.
